n.MAS<-function(N,Argt,Nc=0.95,opc=2){
  ...
  if(length(Argt)!=2){stop("Error, Argt length is not valid \n")}
  #...........................................................................
  alfa<-1-Nc
  z<-qnorm(1-alfa/2)
  n.1<-function(S,Emax.a){
    if(S>0 & Emax.a>0){
      no<-as.numeric(((z*S)/Emax.a)^2)
      n<-as.numeric(ceiling(no/(1+(1/N)*no)))
      return(list(n=n,no=ceiling(no)))
    }else{stop("Error, one of the Arguments is invalid \n")}
  }
  n.2<-function(Cve,Emax.r){
    if(Cve>0 & Emax.r>0){
      no<-as.numeric(((z*Cve)/Emax.r)^2)
      n<-as.numeric(ceiling(no/(1+(1/N)*no)))
      return(list(n=n,no=ceiling(no)))
    }else{stop("Error, one of the Arguments is invalid \n")}
  }
  n.3<-function(p,Emax.a){
    if((p>0 & p<1) & Emax.a>0){
      q<-1-p
      no<-as.numeric((((z^2)*p*q)/Emax.a^2))
      n<-as.numeric(ceiling(no/(((N-1)/N)+(1/N)*no)))
      return(list(n=n,no=ceiling(no)))
    }else{stop("Error, one of the Arguments is invalid \n")}
  }
  n.4<-function(p,Emax.r){
    if((p>0 & p<1) & Emax.r>0){
      q<-1-p
      no<-as.numeric(((z^2)*q)/((Emax.r^2)*p))
      n<-as.numeric(ceiling(no/(1+((1/N)*no))))
      return(list(n=n,no=ceiling(no)))
    }else{stop("Error, one of the Arguments is invalid \n")}
  }
